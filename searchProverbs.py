...
async def generate_tasks(queue):
    async with aiofiles.open("Bible-kjv/Proverbs.json", 'r') as file:
    ...
    async for chapter, chapter_contents in tqdm(list(get_chapters(book_contents)), desc="Chapters: ", leave=True):
        async for verse, verse_text in tqdm(list(chapter_contents), desc="Verse: ", leave=False):
            await queue.put((chapter, verse, verse_text))
...
async def process(queue, session, question, yes_token_id, no_token_id, topn=25):
    ...
    while True:
        ...
        contents_string = f"Prob {chapter}:{verse}"
...
async def main():
    ...
    while True:
        ...
        async with aiohttp.ClientSession() as session:
            ...
            print(f'Scores accumulated. Best {len(scores)} proverbs to follow')
            for text in scores:
                print(f"{text['ref']} is score {get_score(text['score'])}:")
                print(text['text'])
                print('\n')
